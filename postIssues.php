...
* @function Abstract PROTECTED postIssues
* @function PUBLIC getDetailsFromUrl
* @function PUBLIC make_api_call
* @function PUBLIC checkInputParams
*/
abstract class postIssues
{
   /**
...
*/
   public function getDetailsFromUrl($repositoryUrl) {
...
*/
   public function make_api_call($url, $username, $password, $postData = array(), $jsonDecodeData = FALSE) {
     ...
     curl_setopt($ch, CURLOPT_USERAGENT, 'User-Agent: Issue Creation Script');
     if ($jsonDecodeData) {
...
*/
   public function checkInputParams() {
     ...
     if ( ! isset($_SERVER['argv'][7])) {
       ...
       $inputArray['issueDescription'] = trim($input);
     } else {
       $inputArray['issueDescription'] = trim($_SERVER['argv'][7]);
     }
   return $inputArray;
 }
}
...
/**
* Class Name: PostGitHubIssue
* Child Class that extends postIssues Abstract Class
* Used for posting Issue to GitHub
* @function PUBLIC postIssue
*/
class PostGitHubIssue extends postIssues
{
 /**
...
/**
* Class Name: PostBitBucketIssue
* Child Class that extends postIssues Abstract Class
* Used for posting Issue to BitBucket
* @function PUBLIC postIssue
*/
class PostBitBucketIssue extends postIssues
{
 /**
...
try {
 $inputData = postIssues::checkInputParams();
 $repositoryUrlDetails = postIssues::getDetailsFromUrl($inputData['repositoryUrl']);
